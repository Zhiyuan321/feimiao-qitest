...
**随包默认仍是模拟适配器。已内置485只读状态驱动，控制数据区目录仍不等于完整的控制驱动，尚未经实机验证。模板不是一个已完成的厂家驱动。** 不猜测帧格式、寄存器、安全范围或接线；从 [设备目录说明](../../src/device/使用说明.md) 开始定位。
...
2026-09-08补充：485适配器已在Qt5.15.2/MinGW8.1 Release下编译，协议、设备控制器及485界面专项测试通过；不代表Win7实机已通过。打包入口为scripts/build_win7_qt5152.ps1，详见根目录Windows7编译说明。
