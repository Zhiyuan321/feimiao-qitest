...
**随包默认保留轻量预览。已内置485只读状态驱动，控制数据区目录仍不等于完整的控制驱动，尚未经实机验证。模板不是一个已完成的厂家驱动。** 不猜测帧格式、寄存器、安全范围或接线；从 [设备目录说明](../../src/device/README.md) 开始定位。
...
485正常回读不代表质谱采集已接通，`connected=true` 时 `ready` 仍为false，真实谱图采集入口保持阻断。网口已接入状态、`0x82` 气压单包与调谐启停的已知字段，完整谱图帧仍未开放。
