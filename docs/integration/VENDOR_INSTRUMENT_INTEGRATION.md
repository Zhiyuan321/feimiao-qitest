...
除状态查询外，设为当前Fullscan方法时还会依次发送TD温度0x02、离子阱温度0x13、EFC流量0x12、抽气PWM 0x04、进气PWM 0x14；每项只在收到对应11/1100应答后继续。12/1200、超时、断线或写失败均停止剩余队列。离子源电压没有485写命令，非零方法值拒绝，不使用网口ESI命令替代。其他加热、泵和电源控制仍未开放。
