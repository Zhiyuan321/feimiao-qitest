...
现有网口TCP与485可并行；状态、气压、调谐及Fullscan方法下发的协议规则和待确认项见 [网口首版接入说明](网口首版接入说明.md)。

## 交付状态与边界

两端程序已经把开关统一路由到 `IInstrumentAdapter::requestSetting`，包含用户确认、一次一条命令、5 秒回执超时、迟到回执丢弃、真实回读检查及审计。

**随包默认保留轻量预览。已内置485状态及Fullscan所需五项基本设置，并通过网口0x81下发质谱参数；本轮仍未经实机台架验证。其他控制数据区目录不等于已经接入。** 不猜测未给出的帧格式、寄存器、安全范围或接线；从 [设备目录说明](../../src/device/README.md) 开始定位。
...
已按《便携式质谱485通讯协议(2).pdf》接入串口状态查询和回读显示。无需厂家插件即可使用；打开串口后仍保留预览，收到并校验有效状态帧才切换为485只读适配器。当前没有实机联调结果。
...
5. 也可启动前设置 `QITEST_RS485_PORT=COM3`（COM3只是示例，必须替换为实际端口）。它与 `QITEST_INSTRUMENT_PLUGIN` 互斥；指定485后在有效回读前保留预览。厂家插件加载失败仍报错退出，不自动回退。
...
解码检查头、地址、长度、尾、状态值及电流/真空规/PWM范围；支持拆帧、连续帧和数据内的55/AA。2026-09-16按COM4旧软件抓包兼容，高压字段保留完整uint16原值，不再以原值5000为上限拒绝整帧；现场新旧软件均收到37887附近原值。忽略查询回显及其他命令，不将1字节控制ACK当状态帧。协议没有CRC，结构检查不能发现所有传输位错误。

除状态查询外，设为当前Fullscan方法时还会依次发送TD温度0x02、离子阱温度0x13、EFC流量0x12、抽气PWM 0x04、进气PWM 0x14；EFC写入按2026-09-17用户修正采用流量乘10的两字节大端数（0～50 mL/min，分辨率0.1），不采用原文档乘1000；回读除200保持不变。每项只在收到对应11/1100应答后继续。12/1200、超时、断线或写失败均停止剩余队列。离子源电压没有485写命令，非零方法值拒绝，不使用网口ESI命令替代。其他加热、泵和电源控制仍未开放。

485正常回读不代表质谱采集已接通，`connected=true` 时 `ready` 仍为false，真实谱图采集入口保持阻断。网口已接入状态、`0x82` 气压单包、调谐启停与Fullscan 0x81方法设置；完整谱图帧仍未开放。

## 稳定数据通道

现有工程师接口是稳定引擎的唯一入口，不另建一套绕过 `IInstrumentAdapter` 的 UI 通信。当前约束如下：

- 收包先验证帧头、长度、CRC/结构和帧尾，非有限值不进入显示；未确认字段保持“未提供”，不用0填充。
- TCP单次读取最多64 KiB，声明帧长最多1026字节；气压单包最多500点。最近收发帧只保留256条，连接事件只保留64条，防止异常流量无限占用内存。
- 网口状态通知已合并限频，图表重绘最多每16 ms一次；无新数据时不维持空转动画。
- 失联、超时、新连接替换旧连接时先使旧快照失效。调谐命令超时后隔离旧连接，不自动重发高风险指令。
- 普通数值保持定点格式；极大或极小的有限值自动改用科学计数法，NaN/无穷大显示“未提供”，不得把超长字符串挤出Windows 7侧栏。

下一阶段真实谱图引擎应继续沿用该边界：设备线程验帧和生成带序号/时间戳的不可变快照，有界队列只保留最新帧，科学计算使用完整快照，UI只读降采样副本。在厂家谱图帧、丢包规则和采集时序未确认前，不实现这部分映射。
...
插件接口现包含只读能力、缓存状态通知及方法参数校验/请求/回读，ABI 标识升级为 `cn.feimiao.InstrumentPlugin/1.2`。厂家插件须按新头文件同步重编译；旧 1.0/1.1 插件不会当作兼容接口加载。
...
8. 当前单机交付不设置登录或账户角色；最终硬件互锁必须由设备自身执行，软件仍保留确认、超时和回执匹配。
...
```text
点击 → 类型/厂家互锁校验 → 真实设备人工确认 → 发送（控件暂不可重复提交）
                                                ├→ 匹配回读 → 更新开关 / 审计
                                                ├→ 拒绝/回读不符 → 未确认 / 审计
                                                └→ 5秒超时 → 状态未知 / 不自动重试
```
