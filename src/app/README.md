...
- 实机开始检测的 checkDetectionStart：当前方法须与当前连接成功回传的参数一致，本次设置仍在等待或失败时不能沿用旧成功；真空度须有有效网口回读且0 < p < 0.01 mbar（E-03及更低压力）；离子阱实测温度须有限且处于83.0～87.0℃（85±2℃，包含两端；2026-09-18用户更新）。入口预检和实际启动均调用，未达标不建采集会话、不清除旧结果、不发开启指令。detectionStartRejected一次携带全部失败原因；离线预览和历史数据重新分析保留原流程。
...
2026-09-18姓名与报告：startSampleDetection在启动前拒绝空白person_name；采集记录保留原姓名及适配器仪器型号。报告路径与界面归档通过support/NamedOutputPath.h生成，处理Windows非法字符、保留名称及同名序号，不覆盖已存在文件。报告被检人来自本次记录，不能套用当前输入。

2026-09-18当前结果隔离：通过预检、建立新采集会话后clearCurrentResult清除内存currentRun/result并通知报告页；网口、模拟和显式导入重分析共用。取消/失败不恢复旧结果，预检拒绝不清除现有记录。历史数据库不删除，显式打开仍可查看。
