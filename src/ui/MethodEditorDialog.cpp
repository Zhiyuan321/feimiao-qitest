...
MethodEditorDialog::MethodEditorDialog(const QString &initialName,const QJsonObject &initial,
        std::function<bool(const QString &,const QJsonObject &)> persist,QWidget *parent):QDialog(parent) {
    ...
    auto *notice=new QLabel("方法参数"); notice->setToolTip("空白表示未指定；保存参数不会直接下发仪器。"); layout->addWidget(notice);
    auto *mode=new RoundedComboBox; mode->addItems({"Fullscan","SIM","MS/MS"}); mode->setCurrentText(initial.value("scan_mode").toString("Fullscan")); layout->addWidget(mode);
    auto *tabs=new QTabWidget; layout->addWidget(tabs,1); QMap<QString,QLineEdit *> edits;
    for(const QString &group:{"基本","扫描","SIM","MS/MS"}) {
        ...
        for(const auto &field:MethodDraft::fields()) if(field.group==group) {
            if(fieldIndex % 5 == 0) {
                body=new QWidget; body->setObjectName("methodFields"); form=new QFormLayout(body);
                form->setFieldGrowthPolicy(QFormLayout::ExpandingFieldsGrow);
                form->setRowWrapPolicy(QFormLayout::DontWrapRows);
                form->setVerticalSpacing(6);
                tabs->addTab(body, group + (fieldIndex ? QString::number(fieldIndex / 5 + 1) : QString{}));
            }
            ++fieldIndex;
    ...
    auto *load=new QPushButton("打开文件"); auto *exportButton=new QPushButton("导出文件"); auto *save=new QPushButton("保存新版本"); auto *cancel=new QPushButton("取消");
    ...
    const auto collect=[=](QJsonObject *values) {
        ...
        QJsonObject result{{"scan_mode",mode->currentText()}};
        for(auto i=edits.cbegin();i!=edits.cend();++i) {
            if(i.value()->text().trimmed().isEmpty())continue;
    ...
    connect(load,&QPushButton::clicked,this,[=]{
        ...
        name->setText(title); mode->setCurrentText(values.value("scan_mode").toString());
        ...
        dirty_=false;feedback->setText("方法已载入；编辑后可保存为新版本");
    });
}
