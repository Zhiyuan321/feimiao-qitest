...
在“仪器控制 → 运行状态 → 网口TCP”点击开始监听（默认11000），设备主动连接电脑。只读倍增管电压、真空规原始值和实验状态；可与485同时连接，分别失效。谱图保留原始帧供导出，控制和谱图转换未开放。详见 [网口首版接入说明](../../docs/integration/网口首版接入说明.md)。
...
- NetworkProtocol.*：TCP帧、CRC与14字节状态解码。
- NetworkInstrument.*：TCP监听、读数有效期、原始帧导出与485并行状态。

- Rs485Protocol.*：最新485协议帧与23字节状态解码。
- Rs485Instrument.*：异步串口只读轮询，失败失效并关闭。
