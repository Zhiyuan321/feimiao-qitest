...
在“仪器控制 → 运行状态 → 网口TCP”点击开始监听（默认11000），设备主动连接电脑。读取倍增管电压、真空规原始值、换算后的真空度（mbar）和实验状态；可与485同时连接，分别失效。0x82气压按用户代码换算为V，暂按单包采样序号显示；射频页支持调谐启停。Fullscan 方法在有效状态且检测停止时，通过网口0x81下发并等待应答。完整谱图采集仍未开放。详见 [网口首版接入说明](../../docs/integration/网口首版接入说明.md)。
